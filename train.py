...
def train(args, model, train_loader, val_loader, plotter=None, logger=None):
    ...
    for epoch in range(cur_epoch + 1, args.epochs + 1):
        ...
        if epoch % args.epoch_print_freq == 0:
            acc1, acc5, loss_val, class_correct, class_total = validate(args, val_loader, model, criterion, epoch, logger)
    ...
    # Save best epoch per-class performance to separate txt file
    if best_class_correct is not None and best_class_total is not None:
        per_class_file = os.path.join(args.save_dir, 'best_per_class_accuracy.txt')
        with open(per_class_file, 'w') as f:
            f.write(f'Best Validation Performance\n')
            f.write(f'Best Top-1 Accuracy: {best_acc1:.1f}%\n')
            f.write(f'Best Top-5 Accuracy: {best_acc5:.1f}%\n\n')
            f.write('Per-class Accuracy:\n')
            for c in range(args.nclass):
                if best_class_total[c] > 0:
                    class_acc = 100.0 * best_class_correct[c] / best_class_total[c]
                    f.write(f'  Class {c}: {class_acc:.1f}% ({int(best_class_correct[c])}/{int(best_class_total[c])})\n')
                else:
                    f.write(f'  Class {c}: No samples\n')
...
def validate(args, val_loader, model, criterion, epoch, logger=None):
    ...
    nclass = args.nclass
    class_correct = torch.zeros(nclass).cuda()
    class_total = torch.zeros(nclass).cuda()
    ...
    for i, (input, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        for c in range(nclass):
            ...
            class_correct[c] += ((predicted == target) & class_mask).sum().item()
    ...
    return top1.avg, top5.avg, losses.avg, class_correct, class_total
